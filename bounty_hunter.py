...
class BulletproofBountyHunter:
    ...
    async def enumerate_subdomains(self, session):
        logging.info(f"Launching Asynchronous Subdomain Enumeration for: {self.domain}")
        crt_url = f"https://crt.sh/?q=%25.{self.domain}&output=json"
        try:
            async with session.get(crt_url, timeout=20) as response:
                if response.status == 200:
                    try:
                        ...
                        found = re.findall(r'"name_value":"([^"]+)"', text_data)
                        for sub in found:
                            sub = sub.replace('*.', '').strip().lower()
                            if sub and sub.endswith(self.domain):
                                self.subdomains.add(sub)
                    logging.info(f"[+] Total Clean Subdomains Discovered: {len(self.subdomains)}")
                    self.report_data["subdomains"] = list(self.subdomains)
    ...
    async def analyze_js_files(self, session):
        ...
        secret_pattern = re.compile(
            r'(secret|api_key|token|bearer|credential|aws_signing|private_key)["\']?\s*[:=]\s*["\']([^"\']{6,})["\']',
            re.IGNORECASE
        )
        for js_url in list(self.js_files):
            try:
                async with session.get(js_url) as response:
                    ...
                    for api in apis:
                        clean_api = api.strip()
                        found_route = urllib.parse.urljoin(self.base_url, clean_api)
                        if found_route not in self.discovered_endpoints:
                            logging.warning(f" [API ROUTE DISCOVERED IN JS]: {clean_api}")
                            self.discovered_endpoints.add(found_route)
                            self.report_data["discovered_apis"].append({"file": js_url, "route": clean_api})

                    secrets = secret_pattern.findall(content)
                    for sec in secrets:
                        sec_type, sec_value = sec[0], sec[1]
                        logging.critical(f" [CRITICAL - SECRET LEAK IDENTIFIED]: Bundle {js_url} Parameter: {sec_type} -> {sec_value[:6]}...")
                        self.report_data["leaked_secrets"].append({
                            "file": js_url,
                            "type": sec_type,
                            "snippet": f"{sec_value[:6]}..."
                        })
            except Exception:
                pass
        ...
        try:
            async with session.get(url) as response:
                ...
                for name, cookie in cookies.items():
                    val = cookie.value
                    if val.startswith("rO0AB") or "rO0AB" in urllib.parse.unquote(val):
                        msg = f"Java Native Serialization Object discovered tracking state inside session cookie '{name}'."
                        logging.critical(f" [CRITICAL DESERIALIZATION]: {msg} at {url}")
                        self.report_data["vulnerabilities"].append({
                            "type": "Insecure Deserialization (Java)", "url": url, "details": msg
                        })
                    if "O:" in urllib.parse.unquote(val) and "%3A" in val:
                        msg = f"PHP Native Serialization footprint payload tracked inside parameter cookie '{name}'."
                        logging.critical(f" [CRITICAL DESERIALIZATION]: {msg} at {url}")
                        self.report_data["vulnerabilities"].append({
                            "type": "Insecure Deserialization (PHP)", "url": url, "details": msg
                        })
        except Exception:
            pass
    ...
    def generate_markdown_report(self):
        ...
        logging.info(f"Writing findings into automated reporting layer file: {filename}")
        with open(filename, "w", encoding="utf-8") as f:
            f.write(f"# Enterprise Vulnerability Intelligence Scan Report\n\n")
            f.write(f"- Target Domain Base: {self.report_data['target']}\n")
            f.write(f"- Scan Execution Timestamp: {self.report_data['scan_time']}\n\n")
            f.write(f"---\n\n## 1. Discovered Subdomain Infrastructure Assets ({len(self.report_data['subdomains'])})\n")
            for sub in self.report_data["subdomains"]:
                f.write(f"- {sub}\n")

            f.write(f"\n## 2. Hardcoded Static Secret Leaks Found ({len(self.report_data['leaked_secrets'])})\n")
            if not self.report_data["leaked_secrets"]:
                f.write("No open hardcoded crypto keys or secrets exposed directly within the global static script bundles.\n")
            for sec in self.report_data["leaked_secrets"]:
                f.write(f"### Critical Danger Exposure\n- Source Bundle URL: {sec['file']}\n- Pattern Indicator Tag: {sec['type']}\n- Masked Content Signature: {sec['snippet']}\n\n")

            f.write(f"## 3. Client-Side Discovered API System Routes ({len(self.report_data['discovered_apis'])})\n")
            if not self.report_data["discovered_apis"]:
                f.write("No unmapped hidden system routing strings exposed inside internal frontend files.\n")
            for api in self.report_data["discovered_apis"]:
                f.write(f"- Route discovered inside {api['file']} -> {api['route']}\n")

            f.write(f"\n## 4. Architecture Vulnerability Footprints Identified ({len(self.report_data['vulnerabilities'])})\n")
            if not self.report_data["vulnerabilities"]:
                f.write("No high-risk edge cases or logic reflection vulnerabilities identified out-of-the-box by static behavioral processing tests.\n")
            for vuln in self.report_data["vulnerabilities"]:
                f.write(f"### [{vuln['type']}]\n- Target Vector URL: {vuln['url']}\n- Vulnerability Technical Details: {vuln['details']}\n\n")

        logging.info(f"[+] Operational Security Logging Completed. Report file structure sealed: {filename}")
    ...
    async def hunt(self):
        async with await self.create_session() as session:
            ...
            if not self.discovered_endpoints:
                self.discovered_endpoints.add(self.base_url)

            logging.info(f"Total Unique In-Scope Endpoints Undergoing Processing: {len(self.discovered_endpoints)}")
            await self.analyze_js_files(session)

            logging.info("Spinning Asynchronous Parallel Interception Attack Surface Matrix...")
            tasks = []
            for url in list(self.discovered_endpoints):
                tasks.append(self.analyze_xss_context(session, url))
                tasks.append(self.check_serialization_and_tokens(session, url))
                tasks.append(self.check_gateways(session, url))
            ...
            await asyncio.gather(*[safe_task(t) for t in tasks])
